...
def _default_campus() -> tuple[float, float]:
    lat = float(os.environ.get("CAMPUS_LAT", "23.5615"))
    lon = float(os.environ.get("CAMPUS_LON", "120.4808"))
    return lat, lon
...
def _render_map(
    *,
    ...
    center_layer = pdk.Layer(
        "ScatterplotLayer",
        data=pd.DataFrame({"lat": [center_lat], "lon": [center_lon], "name": ["中大校本部"]}),
        get_position=["lon", "lat"],
...
def run() -> None:
    ...
    with st.sidebar:
        ...
        with st.expander("用地名設定中心（可選）", expanded=False):
            ...
            place_query = st.text_input("輸入地名", placeholder="例：國立中正大學")
            if st.button("查詢並設為中心", key="geocode_btn"):
                coords = _geocode_place_name(place_query)
                if coords is None:
                    st.error("找不到地點，請換關鍵字（可加「民雄」或「嘉義」）。")
                else:
        ...
        radius = st.slider("搜尋半徑（公里）", min_value=0.3, max_value=3.0, value=0.5, step=0.1)
