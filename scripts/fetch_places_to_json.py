...
def main() -> int:
    ...
    parser.add_argument("--lat", type=float, default=23.5615, help="Center latitude")
    parser.add_argument("--lon", type=float, default=120.4808, help="Center longitude")
    parser.add_argument("--radius-m", type=float, default=8000.0, help="Search radius in meters")
    parser.add_argument("--max-pages", type=int, default=5, help="Max API pages per grid cell (20/page)")
    parser.add_argument(
        "--target",
        type=int,
        default=1000,
        help="腳本提前停止上限（非產品 KPI；中大 8 km 建議 1000）",
    )
    parser.add_argument("--grid", type=int, default=10, help="Grid size (10 = 10x10 centers)")
    parser.add_argument(
        "--no-text-search",
        action="store_true",
